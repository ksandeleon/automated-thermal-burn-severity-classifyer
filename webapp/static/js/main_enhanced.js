...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Mobile-specific touch and hold functionality
    if (isMobile()) {
        initializeMobileUpload();
    }

    function initializeMobileUpload() {
        ...
        if (!mobileUploadIcon || !mobileFileInput || !mobileCameraInput) return;
        ...
        mobileUploadIcon.addEventListener('touchstart', handleTouchStart, { passive: false });
        mobileUploadIcon.addEventListener('touchend', handleTouchEnd, { passive: false });
        mobileUploadIcon.addEventListener('touchcancel', handleTouchCancel, { passive: false });

        // Mouse events for testing on desktop
        mobileUploadIcon.addEventListener('mousedown', handleMouseDown);
        mobileUploadIcon.addEventListener('mouseup', handleMouseUp);
        mobileUploadIcon.addEventListener('mouseleave', handleMouseLeave);

        // File input handlers
        mobileFileInput.addEventListener('change', handleMobileFileChange);
        mobileCameraInput.addEventListener('change', handleMobileFileChange);
        ...
        function startHold() {
            ...
            // Visual feedback
            mobileUploadIcon.classList.add('tap-feedback');
        ...
        function endHold() {
            ...
            // If it was a quick tap, trigger file selection
        ...
        function cleanup() {
            ...
            mobileUploadIcon.classList.remove('tap-feedback', 'hold-feedback');
        ...
        function triggerFileSelection() {
            console.log('Mobile: Triggering file selection');
        ...
        function triggerCamera() {
            console.log('Mobile: Triggering camera');
        ...
        function handleMobileFileChange(e) {
            ...
            if (file) {
                ...
                // Sync to main file input
                syncFileToMainInput(file);
        ...
        function syncFileToMainInput(file) {
            try {
                const dt = new DataTransfer();
                dt.items.add(file);
                fileInput.files = dt.files;
            } catch (error) {
                console.warn('Could not sync file to main input:', error);
            }
        }
    }
    ...
    function hideUploadAreas() {
        // Hide desktop upload area
        if (uploadArea) {
            uploadArea.classList.add('d-none');
        }

        // Hide mobile upload area
        const mobileContainer = document.querySelector('.mobile-upload-container');
        if (mobileContainer) {
            mobileContainer.classList.add('d-none');
        }
    }

    function showUploadAreas() {
        if (isMobile()) {
            const mobileContainer = document.querySelector('.mobile-upload-container');
            if (mobileContainer) {
    ...
    function showUploadAnimation() {
        ...
        const activeArea = isMobile() ?
            document.querySelector('.mobile-upload-icon') :
            uploadArea;

        if (activeArea) {
            activeArea.style.transform = 'scale(0.98)';
            setTimeout(() => {
    ...
    document.addEventListener('keydown', function(e) {
        ...
        const activeUploadArea = isMobile() ?
            document.getElementById('mobileUploadIcon') :
            uploadArea;

        if ((e.key === ' ' || e.key === 'Enter') && e.target === activeUploadArea) {
            e.preventDefault();
            if (isMobile()) {
    ...
    const mobileUploadIcon = document.getElementById('mobileUploadIcon');
    if (mobileUploadIcon) {
        mobileUploadIcon.setAttribute('tabindex', '0');
        mobileUploadIcon.setAttribute('role', 'button');
        mobileUploadIcon.setAttribute('aria-label', 'Tap to select image or hold to open camera');
    }
});
...
function showNotification(message, type = 'info') {
    ...
    if (type === 'success') {
        ...
        notification.style.background = 'linear-gradient(135deg, rgba(165, 182, 141, 0.9), rgba(193, 207, 161, 0.9))';
        notification.style.color = 'white';
